...
public sealed class GoalsViewModel : ViewModelBase
{
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            IReadOnlyList<Goal> items = await _goalService.ListActiveAsync(_shellState.CurrentPortfolioId).ConfigureAwait(true);
            ...
            IReadOnlyDictionary<Guid, decimal> baselineByGoal = await _baselineService
                .CalculateCurrentAmountsAsync(_shellState.CurrentPortfolioId, items)
                .ConfigureAwait(true);
