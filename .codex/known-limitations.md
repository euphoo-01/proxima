...
- Module 17 adds PostgreSQL schema/bootstrap foundation and committed SQL migration scripts; runtime AppShell now defaults to PostgreSQL repositories and uses explicit Repository + UnitOfWork infrastructure patterns (Iteration 35-36 / REC-004), while auth profile persistence still uses a dedicated local JSON auth store in current local-first mode.
...
- Design refresh pass (2026-05-05) introduced stronger light-bento palette contrast for key summary cards and gradient accents via shared design tokens; primary content area now renders on a solid white base instead of transparent shell background.
- Until strict Mockup parity is verified through custom `figma_mcp_server`, release readiness must remain `Partial`.
- Runtime AppShell host supports development-only auto-login when `PROXIMA_DEV_AUTO_LOGIN != 0` and explicit credentials are provided via `PROXIMA_DEV_AUTOLOGIN_LOGIN` / `PROXIMA_DEV_AUTOLOGIN_PASSWORD`; runtime no longer contains fixed bootstrap credentials.
- Login/Unlock runtime uses profile-backed local auth (`LocalProfileAuthGateService` + JSON profile store) and binds authenticated user context into AppShell runtime services.
- Password recovery/reset remains intentionally disabled in runtime UI until a dedicated secure recovery flow is implemented.
- First-run local profile setup now uses a dedicated registration form (`RegisterView` + `RegisterViewModel`) with role selection and password policy checks; after profile creation runtime performs authenticated auto-entry into AppShell.
...
- Recovered `Asset Details` runtime screen now uses `AppAssetDetailsReadModelProvider` with DB-backed asset/transaction/quote-cache reads and analytics metric mapping; full historical OHLC provider integration remains deferred to REC-009.
- Recovered `Goals` runtime screen now computes per-goal current baseline from persisted portfolio assets via `IGoalProgressBaselineService`; scenario presets and configurable horizon remain deferred to REC-013.
